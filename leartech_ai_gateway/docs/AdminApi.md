...
> ApiPricingResponse admin_v1_pricing_get()
...
async with leartech_ai_gateway.ApiClient(configuration) as api_client:
    ...
    try:
        # Model rates, including which models have no price on file
        api_response = await api_instance.admin_v1_pricing_get()
        print("The response of AdminApi->admin_v1_pricing_get:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling AdminApi->admin_v1_pricing_get: %s\n" % e)
```
...
[**ApiPricingResponse**](ApiPricingResponse.md)
